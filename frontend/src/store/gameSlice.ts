...
type SessionRoomRevision = { epoch: string; sequence: number };

function isStaleRoomRevision(session: Session, revision?: SessionRoomRevision): boolean {
  return (
...
function applySessionRoomUpdate(
  session: Session,
  room: RoomData | null,
  revision: SessionRoomRevision | undefined,
  scene: SceneSummary | null | undefined
): void {
  if ((session.room?.id ?? null) !== (room?.id ?? null)) {
  ...
  reducers: {
    ...
    setSessionRoom: (
      ...
      action: PayloadAction<{
        character: MyRosterEntry['name'];
        room: RoomData | null;
        revision?: SessionRoomRevision;
        scene?: SceneSummary | null;
      }>
    ) => {
      const { character, room, revision, scene } = action.payload;
      const session = state.sessions[character];
      if (!session || isStaleRoomRevision(session, revision)) return;
      applySessionRoomUpdate(session, room, revision, scene);
    },
