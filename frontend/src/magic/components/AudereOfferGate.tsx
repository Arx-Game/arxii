  ...
  return (
    <>
      <button
        ...
        className="flex w-full animate-pulse items-center gap-2 rounded-md border border-fuchsia-500/60 bg-fuchsia-950/40 px-3 py-2 text-left text-sm font-semibold text-fuchsia-300 shadow-[0_0_24px_-8px] shadow-fuchsia-500/60"
      ...
      <AudereOfferDialog
        ...
        onAccept={() => {
          respond.mutate({ offer_id: offer.id, accept: true });
          setDialogOpen(false);
        }}
        onDecline={() => {
          respond.mutate({ offer_id: offer.id, accept: false });
          setDialogOpen(false);
        }}
