...
│   ├── transition-issue-phase.sh         # verify-after-write phase transition contract
...
- **validate-discovery.sh** `<issue-number> [--body-file PATH] [--labels CSV]` → validates exactly one discovery marker outside the spec markers, the nonempty visible lane-assessment fields, and the fail-closed rule that only a complete lightweight lane may implement without `spec:approved`; spec review validates the full standard/heavyweight PRD packet. Its explicit `--allow-approved-legacy` compatibility mode permits only the numbered issues in `references/legacy-approved-issues.txt`; new phase transitions never use that mode. It emits a concise valid result or exits non-zero.
- **transition-issue-phase.sh** `<issue-number> <from-status-label> <to-status-label>` → validates the owned issue snapshot, performs one status-label transition, re-fetches the issue, and fails closed if the body, discovery marker, labels, assignee, approval, or issue state changed. It does not roll back a conflicting edit; recovery requires inspecting the issue before retrying. Read failures are retried three times; a write failure or post-write read failure requires manual inspection because the label edit may have partially applied.
...
For a `review:evidence-required` issue, `open-pr.sh` requires `PR_EVIDENCE_URL` (a
GitHub comment holding the report, preferred) or `PR_EVIDENCE_FILE` (a committed
