...
the commit as a pass, re-stages auto-fixes only for fully staged files (a pathspec commit
runs its hook against a temporary index, so there it refuses and asks for the same commit
again), and is installed as a shim in the shared `.git/hooks`, so CLAUDE.md now lets up to
three implementers share a worktree when their plan tasks' file lists do not overlap and
each commits with a pathspec (the index is a second shared surface: a plain commit releases
`index.lock` before its hooks run, so a sibling's `git add` in that window lands in it,
while `git commit -- <files>` holds the lock throughout). Rejected: a worktree per
