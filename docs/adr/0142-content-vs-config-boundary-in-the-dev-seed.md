...
check types, thread-pull catalogs) and never authors catalog content again. The one known remnant,
`ensure_portal_travel_content()`'s "Mirrorwalking" `Gift`/`Technique`, was removed in #2967 along
with every seeder-invented `Resonance`; a real portal-travel technique is lore-repo content. Rejected: a synthetic
...
keyed to a `Resonance` the seeder itself invented (demo rooms, the "Hallowed Threshold"
story, the "Social Arts" Gift/Techniques, Corruption templates, reaction conditions +
achievement bridge, the touchstone/reagents, fall-redemption examples, and a lazy-created
"Devotion" `RelationshipTrack`). #2973 moved every one to its ruled destination (lore fixture,
the #2451 grid bundle, or a test fixture) and deleted the `integration_tests/game_content/magic.py`
compatibility facade once its last strip-side re-export was gone. `seed_magic_dev` now authors only
the config this ADR describes; KEEP-side lookups resolve authored content via
`world.seeds.sample_content.authored_or_sample()` (ADR-0168) instead of inventing it.
