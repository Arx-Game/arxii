...
RP leak (silence itself becomes a tell), so those keep #1278's original behavior unchanged. Every
OOC seam this extends (mail, journal reactions except the one neutral-rejection exception,
event invites, kudos/reaction windows, journal feed, pages/tells, friend adds) follows one
uniform mechanism: **write-then-filter, never skip-the-write** — the actor's own write path is
byte-identical to an unblocked/unmuted send, and suppression is exclusion on the *other* party's
read. Pages/tells were previously flag-only for a block (like IC channels); #2996 upgrades them to
delivered-suppressed since a page is genuinely OOC (no scene, no RP-leak risk), reusing the
existing OOC-mute drop mechanism while keeping the pre-existing staff `BlockContactFlag` signal
firing unconditionally. Journal praise/retort responses are the one deliberate exception to
write-then-filter: a block rejects the response outright with a neutral shared failure message,
since "this entry isn't available to respond to" already has many innocent causes and can't leak
a block by itself, whereas creating-then-hiding a response row would still let the responder infer
a block from render-vs-list discrepancies. Rejected: leave account-level narrowing at the players'
own knowledge of the advanced option — this is exactly the shape the spec's Decision 6 anti-derivation
review flagged as silently under-serving the common case ("I block/mute a person, not one face of
them"), and it would have left four of eight enforcement seams unenforceable without a parallel,
