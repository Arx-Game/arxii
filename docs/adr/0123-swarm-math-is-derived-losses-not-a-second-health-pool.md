...
resource: `swarm_strike_modifier` computes a banded flat check penalty for acting against the
swarm straight off the current count (negative like ELITE quality — units never roll
their own offense, so target-side resilience is where mass expresses itself; no
persisted field), and `_apply_swarm_losses` costs bodies
...
surface for a feature whose actual ask (#1841) was "numbers carry weight, and swarms shed bodies
