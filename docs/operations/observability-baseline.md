...
- **Logs and Sentry (built, #3599):** Evennia's `log_err`/`log_trace` output is forwarded to Sentry by a Twisted log observer installed next to `sentry_sdk.init` in `settings.py` (`evennia_extensions/observability/sentry_twisted.py`); the SDK's default dedupe collapses a traceback's per-line events into one issue. Django's own `logging` records are bridged into Twisted's log (`log_bridge.py`) so they reach `server.log`, which in daemon mode they previously never did. In production the log files live in `/var/log/arxii` (`LOG_DIR` env var, created by `roles/app_deploy`, pruned by systemd-tmpfiles) where the `arxops` ops account reads them; see `ops-access.md`. Request bodies are no longer sent to Sentry (`max_request_body_size="never"`).
...
- **Dev reader keys: read-only, backups-bucket-scoped, per-developer, revocable.** A read-only S3 key can `Get`/`List` but structurally cannot `Put`/`Delete` — a local errant agent can at worst *download*, never delete/overwrite, and cannot reach any other bucket or any Linode resource. **Built** (#2236 Phase 4): `just pull-prod confirm=yes` (`infra/scripts/pull_prod_db.sh`) is the one-command realization of this — fetches the latest dump via the dev_reader key and restores it into the local dev Postgres (drop/recreate + migrate). See `infra/README.md` § "Pull prod data down".
- **Writer key is automation-only.** Held only by the on-box nightly `pg_dump` backup job (a systemd-timer on the prod host — *not* GitHub Actions; "CI-only" per §4.8 specifically means the Linode API token, a different credential), never on dev machines. Producer and consumers use different credentials with different rights.
- **Immutability backstop (defends even a mis-scoped/compromised key, including the API token) — DESIGNED, NOT YET BUILT (#2236).** The design called for the backups bucket to carry **versioning** and **Object Lock with a retention window ≥ the backup-retention horizon**, so in-retention backups could not be deleted or overwritten by anyone, with pruning past the horizon as a server-side bucket lifecycle policy. As implemented, Object Lock is **not active**: the pinned Linode provider (`~> 2.20`) exposes no Object Lock argument on `linode_object_storage_bucket`, and Cloudflare's R2 lock resource needs provider `>= 5.4` while this repo pins `~> 4.40` — there is nothing to wire without fabricating a resource the provider doesn't support. Compensating posture in the meantime: the on-box Linode writer key is bucket-scoped (cannot reach any other bucket/account) and the R2 offsite copy is a genuinely independent second copy (separate provider, account, and out-of-band credential), so a compromise of one copy's credential cannot reach or delete the other — see `infra/README.md` § "Known gap: Object Lock" for the full accounting. Neither backstop is immutability against a compromised credential with delete rights on *its own* bucket, which is the residual risk Object Lock would close. Revisit when either provider pin is deliberately bumped past the versions above. *Object Lock, if/when added, would apply to the **backups bucket only**: the Terraform state object lives elsewhere in the same bucket family (§4.3) and must **not** be Object-Locked — the S3 backend rewrites state in place, and locking it would break `tofu apply`.*
- **Relied-on external dependency (not a residual to mitigate here).** Backup download to dev machines is safe **because the database is designed to contain no PII** — an application-level, compliance-driven invariant (data minimization). That invariant is **out of scope for this infra sub-project** but is an explicit dependency of this section: §4.9's dev-download capability is only as safe as that invariant. *Honest caveat for whoever owns the app layer (not this spec):* "no PII in the DB" must be actively enforced and verified, and the hard case for a roleplaying game is **incidental PII in player-authored free text**, not schema fields. Near-term controls that still apply regardless: per-dev revocable keys + access logging. The previously-considered scrubbed/anonymized-dump lever is **dropped** — superseded by the no-PII-at-rest design.
...
10. A per-dev read-only Object Storage key can download backups but provably cannot delete/overwrite them or reach any other bucket or Linode resource; Object Lock prevents deletion of in-retention backups even via a read-write key or the Linode API token. **Status: the read-only-key half is built; the Object Lock half is a known gap** — see §4.9 and `infra/README.md` § "Known gap: Object Lock" (#2236, blocked on a provider version bump).
...
- **Backup download safety depends on an out-of-scope app-level invariant** — dev backup access (for local instances / integration tests) is safe *contingent on* the database containing no PII, which is an application-level, compliance-driven design responsibility outside this sub-project. Tracked as an explicit dependency (§4.9), not an accepted PII-sprawl residual; per-dev revocable keys + access logging are the near-term infra-side controls.
...
- **Media (Cloudinary) durability** — user portraits/crests live only in Cloudinary; DB dumps never covered them. **Built** (#2236 Phase 4): a weekly Cloudinary→R2 mirror (`arxii-media-mirror.service`/`.timer`, part of `roles/offsite_replication`, same R2 bucket/credential as the DB offsite job under a `media/` prefix) closes the previous zero-backup gap — RPO ≈ 7 days on media (accepted; media is comparatively low-churn). Incremental, never deletes; Cloudinary remains the primary/serving copy. See `infra/README.md` § "Media durability".
