...
| Declared strain (#3446) | `CombatRoundAction.strain_commitment` (inherited from `CommittingDeclaration` — the field always existed; #3446 wired it) is validated against `CombatParticipant.available_strain` at `declare_action` and forwarded into `use_technique(strain_commitment=...)` at resolution, adding to effective cost and thus the deficit -> soulfray term. Surfaces: telnet `cast ... strain=<n>`, the focused-declaration strain slider in `YourTurn`/`ActionDeclarationCard`; the scene `ActionPanel`'s pre-existing slider covers standalone casts. |
