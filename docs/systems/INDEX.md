...
- **Unlock Requirements** (all have `is_met_by_character(character) -> tuple[bool, str]`):
  ...
  - `RelationshipRequirement` — counts the character's own open `RelationshipLabel` rows on
    active sides whose claimed `tier >= minimum_tier`, optionally narrowed to one
    `required_type` (FK to `RelationshipType`, null = any), against `minimum_count`
