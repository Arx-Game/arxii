...
- **Play API (narrative reader, #3759):** `/api/play/*` read-only reader contracts
  (`play_views.py`) — conversations, threads, poses, a cursor-paged context window, bounded
  search, plus a read-marking POST — all riding the same `InteractionQuerySet.visible_to`
  authorization the scene feed uses. See scenes.md's "Play API (narrative reader)" section.
