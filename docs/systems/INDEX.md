...
- **Reactive content seeds:**
  ...
  - `ensure_redirect_content()` (`src/world/combat/redirect_content.py`, #2210) —
    ...
    `get_or_create` idiom.
- **Telegraphed enemy wind-ups + reaction economy (#2637, #2639; ADR-0156 — extends
...
- **Sent Flying — consequence events in flight (#2638; ADR-0157 — clones #1228's plummet
  ...
  - **Content** (`world.combat.sent_flying_content.ensure_sent_flying_content`, wired into
    ...
    `ROUNDS`/1, despite the design doc's "1-round marker" shorthand; see ADR-0157) reusing
