...
- `InteractionReply` (#3787) records the reply PARENT edge, separately from
  ...
  reply (unique on the child interaction), modelled on `InteractionReceiver` (see ADR-0291
...
make the reply land (ADR-0291, decision 1: refuse, never promote).
...
- see ADR-0291, decision 3, and the module's own docstring for why (Battle scenes have no
...
fragmenting into one group per victim (ADR-0291, decision 2). Combat ACTION/OUTCOME rows
