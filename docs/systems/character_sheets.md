...
|-------|---------|------------|
| `CharacterEnemy` | Who wants the character to fail, priced (#3621, ADR-0279) | `character`, `kind` (`EnemyKind`), `organization` / `family` / `figure_name`, `power_tier` (`EnemyPowerTier`), `reach` (`societies.EnemyReach`), `degree` (`EnemyDegree`), `price`, `why`, `public_line`, `secret` (nullable), `status` (`EnemyStatus`: placed / pending). Written at CG finalize; `CharacterEnemyAdmin` recomputes the price when staff place a free-written one |
