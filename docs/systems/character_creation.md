...
It seeds: `Realm` "Arx"; `StartingArea` "Arx City" (access_level=ALL); the three authored Arx `Beginnings` — "Caretaker", "Sleeper", "Misbegotten" — plus "Luxen Commoner" (`_seed_beginnings()`; canonical prose + per-beginning species/heritage/family gates live in the lore repo's `beginnings/arx.md` — edit there first, mirror in seeds); `Species` "Human", "Khati", "Daeva", and the "Elf" parent with Nox'alfar/Sylv'alfar/Rex'alfar children (the Elf parent lets `allowed_species` grant "all elves" via parent expansion); `Gender` key `unspecified`; `TarotCard` "The Fool" (MAJOR, rank 0); `HeightBand` `average_band` + `Build` `average_build`; the 12 stat `Trait` rows; the two `Roster` rows ("Available"/"Active Characters"); a `Path` "The Wanderer" (PROSPECT); and, via `_seed_cg_explanations()` (#2162), every `CGExplanation` heading/intro/desc row (`CG_EXPLANATION_COPY`, 28 keys — one per `copy?.<key>` lookup across the 11 stage components) so a fresh deploy never ships blank CG stage copy. Pre-content "Commoner"/"Noble" placeholder rows are retired (`is_active=False`) only when their description still exactly matches the old placeholder text — an edited row is never touched. Unlike the rest of this seeder, `CGExplanation` rows are `update_or_create`d on every run (in-repo prose fixes keep reaching already-seeded deploys); every other row stays `get_or_create` (never overwrites a staff edit). `seed_beginning_traditions()` (#2426 whole-branch-review fix) then links every seeded `Beginnings` row to the magic-seeded "Unbound" `Tradition` via a `BeginningTradition` row (`required_distinction=None`) — without it, `TraditionViewSet` and `select_tradition` have nothing to offer and CG's Tradition step is uncompletable on a fresh DB, even the tradition-agnostic Unbound path. It also links the three Arx beginnings to the city's authored traditions ("The Vigil", "Metallic Order", "Fractals of the Abyss") when those lore-fixture `Tradition` rows exist — they are content, not seed rows, so a fixture-less DB still seeds cleanly with just Unbound. Registered last in `CLUSTER_SEEDERS` (after `magic`, which provides the gift/technique/resonance `finalize_character` picks and the Unbound `Tradition` row itself). Verified by `test_playable_slice.py::TestSeededCharacterCreation` (finalize + the real Tradition-step gates run on a seeded-only DB), `test_character_creation_magic_seed.py` (`seed_beginning_traditions` idempotency + defensive skip), `test_idempotency.py::test_edited_cg_row_survives_reseed`, and `test_clusters.py::test_cg_explanations_seeded_and_nonempty`.
