...
| Character sheet | shipped | **#3898 — the Reference Sheet:** the web sheet is the reference sheet an artist makes for a character, on the Folio system. A night plate (art, name + titles, concept, quote, two glance lines, a clickable looks strip the owner wears) over eight sections replacing sixteen tabs — five public, then a marked break and three the owner alone reads (Knowledge, Holdings, Growth). Gating is render-or-vanish, so the page keeps its shape for a stranger, a friend and the player alike. New payload: `looks` (tenure media tagged with the `MoodOption` each shows) and `plate_ink`. Friends left the sheet for the account. Remaining: the rumor a stranger sees in place of the gated bands is wired to nothing yet (the gossip system exists; which guideline it may draw from is Apostate's call), and Ties / Magic / Knowledge / Holdings / Growth place their existing panels inside the new headings rather than being re-skinned into entries |
