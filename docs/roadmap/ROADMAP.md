...
| Almanach de Catenys | shipped (staff); Plan B founder claims in-progress | **#3983 (ADR-0309–0313):** a staff feudal-ladder builder over `Title`/`Area`/`Domain` — plant/batch/name rungs down to a barony seat (new `AreaLevel` rungs BARONY/COUNTY/DUCHY/EMPIRE between CITY and CONTINENT), liege-by-containment with vassal re-homing, and a staff-only ladder + house-document API/React console. House charter/fealty/demesne/estate/holdings editing, kin + household authoring (with the `believed_deceased` public-belief layer), and publish/unpublish round it out; `HouseState` and a schema-only `Title.claimant_org` land the house-state/contested-title axes, a claim's actual resolution left to a later loop. **Plan B (founder self-serve claims, backend Tasks 1-2 shipped):** `OriginTemplate.max_claim_tier` + `Area.is_capital` gates, `claim_grants` (the claimed chain + loose baronies it swallows), the shared `record_kin` kin-writing engine, nested `HouseClaimKin`/`HouseClaimLand` rows on `submit_house_claim`, and `materialize_house_claim` seating the whole grant + placing the founder-written kin tree + an optional estate — see `docs/systems/houses.md` "Founder claims". The founder-facing serializer + CG UI (remaining Plan B tasks) are not yet built. |
