...
| Tasking (covert orgs & NPC jobs) | in-progress | #2820 phase 1 shipped: `world/tasking` dual-fulfillment primitive — org-issued deadline jobs, NPC agent resolves offscreen via double check (handler dispatch margin × agent roll) into outcome-tier routes; ADR-0092 risk pools; org board API + OrgPage panel. Phases 2–5 (covert-org layer, listener posts, counterplay, PC mission pickup) on the issue. See `docs/systems/tasking.md`, ADR-0174 |
