...
- **#2197 DONE (2026-07-12), supersedes the Phase 11 line above:** the dedicated `/scenes/:id/combat` route (`combat/pages/CombatScenePage.tsx`) is gone. Its right-rail composition was extracted verbatim into `combat/components/CombatRail.tsx` (`{sceneId, encounterId}`), which `SceneDetailPage` now mounts in-scene as the right column of a two-column grid whenever `useEncounterForScene` resolves an active encounter for `/scenes/:id`; `/scenes/:id/combat` now just redirects there. See ADR-0127.
