...
- **Reactive protections narrate their own fizzles and lapses (#3574, ADR-0270).** The
...
- **PC sustained actions — the wind-up mirror, for a PC's own commitments (#2705,
  ADR-0190, extends ADR-0161), SQLite tier, journey-proven.** A `Technique.windup_rounds
  > 0` technique, or a ritual whose `RitualCheckConfig.sustained_rounds > 0`, defers
  instead of resolving same-round: declaring rolls Concentration ONCE
  (`perform_check_with_modifiers`, so the 26 authored `ConditionCheckModifier` rows on
  Concentration actually apply) to set an `absorption_budget = clamp(2 +
