...
**Status:** engine + authoring + play loop + web discovery doors SHIPPED (code-verified 2026-06-11, web doors #3044 2026-08-08); reward sinks / multiplayer surface are the open gaps
...
- **Quest journal + in-progress persistence (#889)**: `/api/missions/journal/` + `JournalPage`/`BeatCard` React play surface; node→room binding (ANYWHERE/ANCHOR/ROOMS) with compass disclosure; durable deed history. **Report affordance ✅ shipped (#3040)**: `JournalPage` gives a RESOLVED entry its own "Awaiting report" section (previously bucketed under "Concluded" with no way to finish it) with a `ReportMissionDialog` offering all four `ReportStyle` choices; a co-location/not-offerable 400 renders the server's own message rather than the client guessing reachability.
...
- **Legend risk floor (#2051 ✅ shipped)**: a `MissionOptionRouteReward` with `sink=LEGEND_POINTS` or a `MissionRenownAward` with legend-paying risk (HIGH/EXTREME) requires the parent template's `risk_tier ≥ LEGEND_RISK_FLOOR_TIER` (4). Hard-blocked at save time via `clean()`; a one-time legacy audit (`arx manage audit_legend_floor`) lists existing violations. Solo characters accepting a legend-risk mission see the darkness warning: "You are alone. Your threads are quiet. This will very likely kill you." See ADR-0107.
...
- **No discovery surface**: ✅ **shipped** (#2044) — `GiverKind.BOARD` is a third giver-kind: an examinable object that lists many eligible postings (preview-then-take). Examine renders a numbered list; `mission take <n>` (telnet) and `POST /api/missions/boards/<pk>/take/` (web) re-run eligibility before granting via the same `staff_assign_mission` path trigger dispatch uses. `GET /api/missions/journal/opportunities/` + the journal opportunities tab surface three groups (here / nearby / your-organizations); trigger givers show flavor only (never their pool — would spoil the draw). Telnet `mission opportunities` mirrors the tab. Discovery is world-anchored (ADR-0094): boards/givers are the doors; the web tab is a map, not a door. **Web doors ✅ shipped (#3044)**: the backend engine above had no mouse-driven web front end — the board client (`getBoardPostings`/`takeBoardPosting`) sat dead with zero consumers, examine-triggered dispatch was unreachable from any live `look` call (telnet included — see the Web front doors entry in `docs/systems/INDEX.md`'s Missions section for the `LookAction`/`at_examined` wiring gap this closed), and NPC givers had no room-view surface. Now: `MissionBoardDialog` opens from a room object's "View Board" button, `ObjectsList` rows expand to dispatch `look` and render the description inline (mission hooks fire identically to telnet), and `NpcGiversBlock` offers "Talk" for any `Functionary` placed in the room, mounting the existing `NPCInteractionDialog`. The Opportunities tab stays untouched — still the map, not a door.
