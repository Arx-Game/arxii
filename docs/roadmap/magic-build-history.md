# Magic System — Build History (Archive)

> **This is the build-log archive, not the status map.** It records *what was built*,
> scope by scope, and exists to prevent reinventing existing surfaces (the
> anti-reinvention record). For **"where are we / what's left"**, read
> [`magic.md`](magic.md) — the concise playable-loop status map. Entries below are
> historical; some "deferred" notes were closed later and may read as stale. Trust the
> status map and the code over this file's framing.
...
- **Capability grant integration:** TechniqueCapabilityGrant links Techniques to CapabilityTypes (from conditions app) with a `base_value + (intensity_multiplier * intensity)` formula. This feeds into the mechanics app's action generation pipeline — when a character has a Technique that grants a Capability, the system can match it against Properties on Challenges to surface available approaches. **Superseded by #2708/ADR-0169 (2026-07)**: `calculate_value()` now curves `base_value` geometrically by real contextual power (technique intensity + context-free power + ambiently-active thread power) once a `CapabilityPowerConfig` row exists; the additive formula above is the inert fallback when no such row exists. See `docs/systems/conditions.md`'s "Capability magnitude curve" section.
