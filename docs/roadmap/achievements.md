...
- **Discovery system:** First-to-achieve tracking with IC/OOC timestamps. Supports simultaneous co-discoverers (e.g., a party all earning an achievement together). "Discovered by Alice, Bob, and Charlie". A Discovery row structurally requires the first earner's current `RosterTenure` (`discovered_by_tenure`, `on_delete=PROTECT`, #3055) — a sheet with no player tenure cannot claim a first-ever slot, since a discovery is a partly-OOC accolade earned through a character, not the sheet in isolation. Simultaneous co-discoverers of the first-ever grant share credit via `Discovery.shared_with_tenures` (M2M); a player's full record is the union of their tenures' primary and shared discoveries
