...
| **Signature technique** (one technique deepened, own resonance, may diverge) | ✅ PROVEN | **ADR-0056** (re-scope `TargetKind.TECHNIQUE`); `cast_resonances_for` per-technique seam replaces `gift_resonances_for` at the four cast sites; cumulative variant matching (gift baseline + signature depth); discordant signature override + one-active-per-technique constraint; `test_signature_technique_e2e.py` (#1582) | done |
