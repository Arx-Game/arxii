...
| **Distinctions acquired/ranked up in play (post-CG)** — closes the "no post-CG distinction acquisition" gap | ✅ PROVEN | **#2037**: single seam `world.distinctions.services.grant_distinction` (origin = first-acquisition provenance, never rewritten by a rank-up), four ratified sources — GM action `gm_award_distinction`/telnet `grant_distinction`, achievement `RewardType.DISTINCTION`, consequence-pool `EffectType.GRANT_DISTINCTION`, and resonance `DistinctionResonanceRankThreshold` (`ENDORSEMENT_THRESHOLD`, sustained in-character endorsement play); see `docs/systems/distinctions.md` "Post-CG acquisition" | done |
...
| Gain a **new Gift** in play (post-CG) | ✅ PROVEN | **Path-crossing grant ✅ PROVEN** — crossing into a new Path mints its Gift(s) via `PathGiftGrant` + `grant_path_magic` (#1579, `test_path_crossing_grant_e2e.py`). **Weave/XP-buy acquisition of a standalone Minor Gift** (**ADR-0050** + **ADR-0053**) now has a player-facing surface (#2116): `PurchaseGiftUnlockAction` (key `purchase_gift_unlock`), telnet `learn gift <id>`, web `POST /api/magic/gift-unlocks/purchase/`. | MVP |
...
| **Learn / train / buy** an existing technique | ✅ PROVEN | **ADR-0056** (signature thread for one technique) + **ADR-0053** (unlock gate) + #2116 wiring: `AcceptTechniqueOfferAction` (key `accept_technique_offer`), telnet `learn technique <id>`, web `POST /api/magic/technique-offers/accept/`. E2E: `test_gift_acquisition_action_e2e.py`. | MVP |
...
| Trainer / teaching system | ✅ PROVEN | **NPC-trainer path ✅ PROVEN (#2440)**: `OfferKind.TRAIN` + `world.npc_services.effects.run_train_offer` — an Academy (or Great Archive) trainer `NPCRole` teaches a specific technique for AP + coin + a Golden Hare, gated on an OWED Academy `OrganizationObligation` (#2428) and on the learner's own (Path × Gift) pool ∪ Tradition-signature availability (`NPCRole.teaches_tradition` scopes signature members-only). Delegates acquisition to the same `charge_and_learn` core `accept_technique_offer` (player-to-player teaching) uses — one seam, two front doors. Player-to-player path remains `MentorBond` (a combat bond, not teaching) for the social/mentorship axis specifically. **Great Archive self-study ✅ PROVEN (#2440 ruling 5, Task 7):** a "Great Archive Librarian" `NPCRole` (seeded via `ensure_great_archive_librarian_role`) offers the same TRAIN-shaped deal, gated by a quest-completion `Achievement` through `NPCServiceOffer.eligibility_rule`'s existing `has_achievement` leaf — no new FK. Real quest content is lore-repo authored; this ships the gate + a small PLACEHOLDER offer set. **Level-2 gate ✅ PROVEN (#2440 ruling 4):** `MajorGiftTechniqueRequirement` — knows >= 3 techniques of the character's MAJOR gift (a count, not completeness) — registered in the `spends.py` requirement-type registry and seeded onto the level-2 `ClassLevelUnlock` via `seed_major_gift_technique_level_requirement`. | done |
