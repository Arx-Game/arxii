...
| **2J — Relationships library** | The `RelationshipType` catalogue (eighteen PLACEHOLDER types in five families, with counterparts) and the single `RelationshipTier` ladder (#3957) | Declare a label. Depth from a shared scene and from the weekly AP turn. Claim a tier with a capstone entry. |
