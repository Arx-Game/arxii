...
| ✅ **1.8** — ~~Cantrip starter catalog via `seed_cantrip_starter_catalog()`~~ **Superseded (#2426 Task 7):** `seed_starter_gift_catalog()` — 5 TechniqueStyle rows, 6 EffectType rows (each stamped with a `TechniqueCategory`), 5 MAJOR Gift rows + 25 authored Technique rows (5 per gift), 5 PathGiftGrant rows, 5 PROSPECT Path rows, the Unbound Tradition + its 5 TraditionGiftGrant rows. No Cantrip rows are seeded anymore — the CG magic stage now picks a catalog Gift + Techniques instead of a Cantrip template (`Cantrip` model removed in Task 8). | CG magic stage works on a fresh dev install. New characters pick a starter Gift + techniques from their Path/Tradition's availability pool. |
