...
**Status:** Phase 1 complete; Phase 2 in-progress; Phase 3 core delivered (3.1–3.7, 3.9–3.10 done; 3.8 optional); Phase B (#1221 tuning/ops dashboards) done
...
| ✅ **1.8** — ~~Cantrip starter catalog via `seed_cantrip_starter_catalog()`~~ **Superseded (#2426 Task 7), then retired (#2474):** `seed_starter_gift_catalog()` used to lazy-create 5 TechniqueStyle rows, 6 EffectType rows (each stamped with a `TechniqueCategory`), 5 MAJOR Gift rows + 25 authored Technique rows (5 per gift), 5 PathGiftGrant rows, 5 PROSPECT Path rows, the Unbound Tradition + its 5 TraditionGiftGrant rows. **#2474 retired this function entirely** — that catalog is now real lore-repo content, loaded via `core_management.content_fixtures.load_world_content()` ahead of every cluster seeder in `seed_dev_database()`, not synthesized in-repo. Consumers (`world/npc_services/seeds.py`'s Academy trainer roles) now read the loaded catalog via `Technique.objects.filter(name__in=...)`, logging a warning (not raising) when it's absent rather than falling back to synthesizing rows. No Cantrip rows are seeded anymore — the CG magic stage picks a catalog Gift + Techniques instead of a Cantrip template (`Cantrip` model removed in Task 8). | CG magic stage works on a fresh dev install (given a loaded content repo). New characters pick a starter Gift + techniques from their Path/Tradition's availability pool. |
...
| ✅ **1.10** — Magic-story pipeline slice (resonance-environment-based): `AffinityInteraction` rows (9 directed pairs) + `ResonanceEnvironmentConfig` singleton seeded into `seed_magic_dev()`; rooms seeded via `tag_room_resonance` (cascade substrate). Universal-path redesign (2026-05-16): dropped "Magically Attuned" condition + universal trigger + universal flow seeding; replaced with OPPOSED `ConsequencePool`/`Consequence`/`ConsequenceEffect` rows for the celestial-place pairs (#4/#7) and ALIGNED `ResonanceAlignmentBoonTier` rows with named buff `ConditionTemplate`s for the abyssal/abyssal pair (#5). Pipeline test `test_magic_story_pipeline.py` — subtests cover OPPOSED backfire (4 `CheckOutcome` tiers × 2 place-intensity bands via the real `resonance_environment_for_cast` orchestrator call), ALIGNED presence buff apply/clear/swap via `refresh_resonance_alignment`, Quiescent (no `CharacterAura`) inert case, CASTER_DOMINANT stub (fill-in for deferred defilement), and second-earner Discovery. `setUp` applies no condition — exercises the production path directly. | Magic-story resonance-environment pipeline provably works seed-through-pipeline on the production code path. The universal-path redesign removes the marker-condition anti-pattern; magic-capability is now derived from `CharacterAura` presence. The primitive (`evaluate_resonance_environment`) is the shared substrate for deferred presence-escalation, defilement (CASTER_DOMINANT), and combat clash-of-wills. |
| ✅ **1.11** — `game_content/combat.py` now exists with `seed_penetration_contest()` composed into `seed_magic_dev()` step 9 (#767). Seeds the trait-weighted penetration `CheckType` (willpower × 1.00 + intellect × 0.50), the 4-rung `PenetrationOutcomeFactor` ladder (bounce / partial / clean / overpenetration), and the check-scoped penetration `ModifierTarget` so caster-side `CharacterModifier` rows (e.g. "+penetration vs warded foes" distinctions) flow through `collect_check_modifiers` into the penetration contest's `extra_modifiers`. Generic `CHARACTER` source added to `collect_check_modifiers` — any check with a linked `ModifierTarget` gains `+X to <check>` support. Penetration check routed through the shared modifier seam in `_apply_penetration`. | Any check-scoped distinction (e.g. Wardbreaker) automatically applies to the penetration contest without additional wiring. |
...
| **2J — Relationships library** | The `RelationshipType` catalogue (twenty-three PLACEHOLDER types in five families, with counterparts) and the single `RelationshipTier` ladder (#3957) | Declare a label. Depth from a shared scene and from the weekly AP turn. Claim a tier with a capstone entry. |
...
| ✅ **3.9** — Check-resolution spine + playable slice (#651; ladder + direction fixed by #2707, ADR-0166/ADR-0165). `src/world/seeds/checks.py` `seed_check_resolution_tables()` seeds the rows `perform_check` needs to turn a roll into a real `CheckOutcome` — STAT/SKILL `PointConversionRange` (1 pt/level over 1-100), the nine-rung `CheckRank` ladder (0/10/25/50/80/115/155/200/250 — widened from the original four rungs by #2707 so a level-30 character, who now carries a guaranteed `LEVEL_POINTS_PER_LEVEL`-per-level term on every check, doesn't pin at the old top rung from level 10 onward), the five-outcome catalog (Critical Failure…Critical Success), and the thirteen `ResultChart` rows (rank-diff −6…+6, widened alongside the ladder) with their per-roll `ResultChartOutcome` bands — all via `get_or_create` on natural keys (idempotent). #2707 also fixed an inverted difficulty gradient in the seeded chart content: `rank_difference = roller_rank - target_rank` was always the engine's convention (the field's own `help_text` said so), but the bands attached to each value had the gradient backwards — an Expert roller against a Trivial target succeeded only 15% of the time pre-fix, an Incompetent roller against an Expert-level target 80%. Nothing had asserted chart direction for the life of the project; `world/checks/tests/test_chart_direction.py` now guards it. The checks spine is the **single authority** for the global resolution charts/outcomes: the magic cluster's `_seed_endure_hallowed_ground_check()` calls `seed_check_resolution_tables()` rather than seeding its own `rank_difference=0` chart (which previously collided on the diff=0 key with contradictory bands). Values are initial sane defaults aligned with the integration-test setup (`PerformCheckTests` / the traits tests), not promoted verbatim from a single factory (Phase B #1221 makes them tunable). Registered as the `"checks"` cluster in `CLUSTER_SEEDERS` (seeded first). `test_playable_slice.py` proves a fresh `seed_dev_database()` resolves a factory character's check (against the seeded `flee` CheckType) to a real non-null `CheckOutcome`. | A fresh seeded DB resolves a real check to a real outcome — the check-resolution spine is provably playable, level-aware on both sides of a check, and direction-correct. |
| ✅ **3.10** — Character-creation world content + admin Game Setup hub (#1333). `src/world/seeds/character_creation.py` `seed_character_creation_dev()` seeds the CG-"world" content a fresh DB needs to run character creation: `Realm` ("Arx"), `StartingArea` ("Arx City", access_level=ALL), `Beginnings` ("Commoner" + allowed_species M2M), `Species` ("Human"), `Gender` (key `unspecified`), `TarotCard` ("The Fool", MAJOR/rank 0), `HeightBand`/`Build` (average), the 12 stat `Trait` rows, the two `Roster` rows ("Available"/"Active Characters"), and a `Path` ("The Wanderer", PROSPECT) — all via `get_or_create` (idempotent, never overwrites edits). Registered as the `"character_creation"` cluster, seeded last (after `magic`, which provides the gift/technique/resonance `finalize_character` picks). `test_playable_slice.py::TestSeededCharacterCreation` proves `finalize_character` runs end-to-end against a seeded-only DB; `test_idempotency.py::test_edited_cg_row_survives_reseed` proves non-overwrite. `seeded_models_by_cluster()` exposes a per-cluster inventory (one key per registered seeder) for the admin hub. The hub itself — `admin_game_setup` view + `admin/game_setup.html` — is a superuser-only landing page ("Welcome to a new Arx-based instance") with the clone→seed→tweak→export flow, a per-cluster content inventory with live row counts, and links to the Big Button, Export/Import, and the World authoring apps. The Big Button's post-seed redirect now lands on the hub. | A clone host seeds CG-world content in one click and has a wayfinding hub to configure their game; a fresh seeded DB can run character creation end-to-end. |

✅ **Phase B (#1221)** — "makes them tunable," referenced at 3.9 above, has shipped: admin-hosted
Game Tuning (`/admin/_tuning/`) and Game Ops (`/admin/_ops/`) dashboards, plus a superuser
content-repo load surface (#1220). See [docs/systems/tuning.md](../systems/tuning.md) and
