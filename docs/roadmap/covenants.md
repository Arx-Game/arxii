...
**Status:** in-progress (Slice A entity + membership FK + engagement context shipped; Slice B RitualSession primitive + formation ritual + engagement UI shipped; Slice D covenant progression + Story integration shipped; Slice E Battle covenants + Durance×Battle combat-precedence shipped; Slice F covenant rites shipped including role-aware level-banded severity-scaling stat packages (#753); per-role powers (#751: tier-0 passive capability application surface + per-(role,resonance) `ThreadPullEffect` catalog) shipped; rite stat-buffs now flow into checks (#783); battle/group-ability/role-power/promotion frontend (#518) shipped; covenant rank passive bonus (#762: authored `CovenantLevelBonus` config, engagement-gated, level-scaled, derive-on-read via `covenant_level_bonus` in the modifier pipeline) shipped; exit lifecycle — voluntary leave + leader-gated kick + below-2 auto-dissolve, soft-only (#519) — shipped; Slice G use-based COVENANT_ROLE anchor cap (#517: additive legend-earned-in-role + time-held-in-role on top of the covenant-level floor, derive-on-read, no migration) shipped; the Slice G use-based weave gate still post-MVP; rank ladder — `CovenantRank` per-covenant authority tier, two-axis `CovenantRole`/`CovenantRank` model, rank management services, `CovenantRankViewSet` API, rank-ladder UI (#1027) — shipped; covenant-role armor-soak gate — compatible→additive, incompatible→`max(physical, resonant pool)`, level-scaled; #1174 — shipped; resonance sub-role runtime resolution (derive-on-read via `resolve_effective_role` + `fire_subrole_discoveries` discovery beat; `discovery_achievement`/`codex_entry` FKs on sub-role `CovenantRole`; `anchor_role` API field; #1277) — shipped; telnet membership lifecycle — `CmdCovenant` (`covenant engage/disengage/leave/kick/rank/transfer/standdown`), seven `action.run()` REGISTRY Actions in `actions/definitions/covenants.py`, `world.covenants.selectors` shared by Actions + viewsets, covenant induction + banner-call rise via `CmdRitual` adapter registry (`commands/ritual_adapters.py` — `CovenantInductionAdapter` + `BannerCallAdapter`) (#1346) — shipped; combat-identity blend — `CovenantRole.archetype` single-enum replaced by `sword_weight`/`shield_weight`/`crown_weight` (sum to 1 on primaries, sub-roles delegate via `blend_weight_for`), `CovenantRoleActionScaling` replacing `ArchetypeActionScaling`, always-on `covenant_role_blend_power_term` baseline cast power term — Layer 1 of ADR-0149's four-layer vow-power model (#2529) — shipped; per-vow finer technique specialty — `CovenantRoleTechniqueSpecialty` (NK `(covenant_role, function)`, valid on primaries AND sub-roles, sub-role rows ADD) + always-on `covenant_role_specialty_power_term`, keyed on the shared code-defined `magic.TechniqueFunction` vocabulary also consumed by Layer 4's situational perks — Layer 2 of ADR-0149 (#2443) — shipped; defense styles + gear substitution — `DefenseStyle` (GEAR_SOAK/EVASION/BARRIER) + per-role `CovenantRoleDefenseProfile.gear_additive_tenths`, `gear_additive_fraction` scaling the compatible-armor bucket once at the #1174 soak seam, `VowGearScaling` removed (subsumed by the profile fraction) — Layer 3 of ADR-0149 (#2533) — shipped; Layer 4 slice 1 of 3 — the deterministic situational-perk machinery: code-defined `Situation` library + evaluator registry, `VowSituationalPerk`/`VowSituationalPerkSituation`/`VowSituationalPerkRung` authoring models, `POWER_BONUS`/`CHECK_BONUS` delivery, and the dual-dispatch (WS + telnet) announce presentation contract — ADR-0151 (#2536) — shipped; Layer 4 slice 2 of 3 — outcome guarantees: `VowSituationalPerk.floor_success_level` (authored, canonical −10..+10 scale, `TIER_FLOOR`-only), the centralized `world.checks.constants.BOTCH_SUCCESS_LEVEL_MAX` botch boundary, multi-kind `applicable_perks(effect_kind=tuple[...])`, and `TIER_FLOOR`/`BOTCH_IMMUNITY` wired into `perform_check`'s outcome resolution (`_apply_outcome_guarantees`, both the rolled and test-rig forced paths) — absolute (never thread-scaled), ungated, announce-on-bind-only — plus the covenant-mate reversal (ally `COVENANT_ALLIES`/`WHOLE_GROUP` group-perks now scope on membership + co-presence, the mate's own `engaged` flag is irrelevant — no death-spiral) — ADR-0152 (#2536) — shipped; Layer 4 slice 3 of 3 — Court/Battle situation scoping + dormant-vow messaging: three `VowSituationalPerk` scope columns (`mission_category`/`mission_template`/`battle_action_kind`) + `perk_scope_matches` (AND semantics, per-`effect_kind` `clean()` gates); `situation_ctx` threaded into all six mission `perform_check` sites (`world.missions.services._situation.mission_situation_ctx`) and every Battle warfare-roll check/cast (`world.battles.resolution._battle_situation_ctx`); five new `Situation` values — `CHAMPION_DUEL` (`CombatEncounter.is_champion_duel`, stamped by `open_champion_duel`), `COMBAT_OPENED_FROM_PARLEY`/`AMBUSH_UNDERWAY` (`CombatEncounter.opened_from_parley`, stamped by `seed_or_feed_encounter_from_cast` on CREATE only, both v1 approximations), `ALLY_INTERCEPTED_FOR_ME` (declared-guard INTERPOSE semantics), `ATTACKER_AFFINITY` (authored `CombatOpponent.affinity` then `CharacterAura` fallback); the defense-side seam — `SituationContext.attacker` threaded through `applicable_perks`/`_PerkResolver` and `resolve_npc_attack`, making CHECK_BONUS/TIER_FLOOR/BOTCH_IMMUNITY perks live on defense rolls for the first time; and dormant-vow messaging (ruling 2's "loud OFF state" — `dormant_perk_firings`/`announce_dormant_perks`, holder-only WHISPER + telnet, wired into all three fired-perk seams) — ADR-0153 (#2536) — shipped. **Layer 4 (four-layer vow-power model, ADR-0149) is complete; #2536 closed.** #2623 follow-up (ADR-0154) parameterized the situation library: `SituationRequirementMixin`'s four typed columns (`threshold_percent`/`count_threshold`/`affinity`/`origin_side`, no JSON — ADR-0007) shared by `VowSituationalPerkSituation` AND `VowSituationalPerkRung`, a per-situation `SITUATION_PARAM_SPECS` allowed/required contract enforced by `clean()`, the Abyssal-only attacker-typing situation renamed to `ATTACKER_AFFINITY` and made authorable against any `AffinityType` axis, and `CombatEncounter.initiated_by_pc_side` (nullable — recorded honestly, no NPC-initiated encounter-creation path exists yet) feeding `origin_side`-gated `AMBUSH_UNDERWAY`/`COMBAT_OPENED_FROM_PARLEY` perks — shipped; all 14 pre-#2623 situation rows behave byte-identically (parameterless = each family's parameterless default). #2646 follow-up added `ON_CHOSEN_GROUND` (15th situation): `room_features.PreparedGround` (OneToOne `prepared_by` — one active prepared ground per character, re-preparing elsewhere MOVES it) recorded by `record_ground_preparation_from_cast`, a rider ("the vow never hands you a new verb") on an out-of-combat standalone cast of a PERCEPTION-tagged technique gated on a new `CovenantRole.prepares_ground` data-authored flag; `world.combat.chosen_ground.compute_on_chosen_ground` stamps the new `CombatEncounter.on_chosen_ground` at CREATE time in the three PC-vs-NPC encounter-creation seams (`seed_or_feed_encounter_from_cast`, `create_lethal_duel`, `open_place_encounter`; `create_pvp_duel` deliberately unstamped) — shipped.) The Sphinx of Black Quartz (#2640, ADR-0157) — read-only, never-gating vow-suitability oracle: `world.covenants.sphinx.judge_vow` (three-tier `SphinxTier.TAKES`/`DORMANT`/`NOT_YET` verdict re-running the kit∩demand join as a report, code-defined `SITUATION_CREATOR_FUNCTIONS` reverse-provenance mapping, bounded shopping list) + `audit_vow_coverage` (staff coverage-audit instrument, `_sphinx/` admin page) + `GET /api/covenants/roles/sphinx/?role=` + the `sphinx <vow name>` telnet command — shipped. Secondary covenant vows — `CharacterCovenantRole.is_secondary` (at most one engaged PRIMARY + one engaged SECONDARY per (character_sheet, covenant type), `set_engaged_membership(as_secondary=...)`, engage-time validation requiring an engaged primary of the same type + a distinct anchor role + a capped COVENANT_ROLE thread level), the four-layer split (Layers 1/3 strictly primary-only via `currently_engaged_primary_roles()`; Layers 2/4 read both standings and scale a secondary-sourced contribution by the new `SecondaryVowConfig.potency_tenths` dial, seeded ×0.6), depth-stays-primary-only (a secondary resolves at its anchor role only — no sub-role graduation, no deep rungs), and secondary-sourced `TIER_FLOOR` guarantees binding one tier weaker (`BOTCH_IMMUNITY` unweakened) — ADR-0159 (#2641) — shipped. The Insight — the Know need's once-per-fight ace, distinct from Layer 4's always-deterministic perks: `InsightTableEntry` (NK `["name"]`, lore-repo content, never seeded in arxii) is a curated weighted-random table of large, narrowly-scoped, ONE-ROUND conditions (never instant-win, enforced editorially by curation); `CovenantRole.grants_insight` is the rider flag (rides an engaged parent role too); `CombatParticipant.insight_used` is the once-per-encounter stamp; `world.covenants.insight.maybe_produce_insight` fires when an engaged `grants_insight` role's PERCEPTION-tagged technique cast resolves in combat, resolving SELF/ALLY/TEAM targets and announcing on both the web broadcast and telnet channels — #2645 — shipped. Minor covenant membership + attrition-dissolve removal + covenant treasury (#2992, ADR-0213) — `CharacterCovenantRole.standing` (`MembershipStanding.CORE`/`MINOR`), a guest tier riding the secondary-vow lane (#2641, ADR-0159) rather than a parallel model: MINOR is DURANCE-only, engages the secondary lane only (with a solo-secondary waiver — no engaged primary required), bypasses the level-band join gate, and transitions via `swear_core`/`step_back_to_minor`; the auto-engage trigger gained a second, independent pass filling the vacant secondary lane from MINOR-standing memberships; the attrition auto-dissolve (below-2 active members) is REMOVED — `leave_covenant`/`kick_member` never dissolve, only formation still enforces the two-founder floor (ADR-0042 amended); and a Covenant Treasury (`world.covenants.treasury`, wiring the existing `OrganizationTreasury`) — open deposits, rank-gated withdrawals (`spend_rank_max`), telnet `covenant deposit/withdraw`, and a web `TreasuryPanel` — shipped.
...
— voluntary leave and rank-gated kick, both soft; the below-2 auto-dissolution
half of #519 was REMOVED by #2992, ADR-0042 amendment — see "Covenant Exit
Lifecycle" below), and the two-axis authority model (#1027 — `CovenantRole` for
power, `CovenantRank` for administrative authority; see "Two Orthogonal
Authority Axes" below).
...
**Formation is the only place the two-founder floor is enforced.** Active
membership dropping below 2 (or to zero) no longer auto-dissolves the covenant
— that attrition rule was removed by #2992 (ADR-0042 amendment) — see the
"Covenant Exit Lifecycle (#519)" design decision below.
...
- **REST API** (`/api/covenants/`):
  ...
  - Engage/disengage actions + `RitualSessionViewSet` landed in Slice B.
    Voluntary leave + rank-gated kick landed in #519 (originally paired with a
    below-2 auto-dissolve, REMOVED by #2992 — ADR-0042 amendment); kick
    re-gated on `CovenantRank` capability + tier precedence in #1027 (see "Two
    Orthogonal Authority Axes" above). Deposit/withdraw against the covenant
    treasury (`deposit_covenant_funds`/`withdraw_covenant_funds`) landed in
    #2992.
...
- **Rank-gated kick** — `kick_member` lets a member whose `CovenantRank` has
  ...
  full two-axis model.
- **No attrition auto-dissolution (REMOVED by #2992, ADR-0042 amendment)** —
  originally, active membership dropping below 2 (by leave or kick)
  auto-dissolved the covenant immediately. That rule is gone: covenants are
  magically core to their characters and accrue shared legend
  (`CovenantLegendCredit` → covenant level); destroying that identity through
  member churn was an artifact of thinking of covenants as ad-hoc clubs. A
  covenant now persists at any active-member count, including zero (an empty
  covenant is inert but its legend endures; a lone core member with an
  engaged minor guest is not "dark" at all). Formation still requires two
  founders — see "Covenants Are Group-Only" above; that floor is unaffected.
...
- Exit lifecycle (voluntary leave, leader-gated kick) — DONE (#519; soft-only;
  the below-2 auto-dissolve half of #519 was REMOVED by #2992, ADR-0042
  amendment — see "Covenant Exit Lifecycle" above)
...
### Minor Membership + Covenant Treasury (#2992) — SHIPPED

- **`MembershipStanding.CORE`/`MINOR`** on `CharacterCovenantRole.standing` — a
  third, orthogonal membership axis (alongside `CovenantRole` for power and
  `CovenantRank` for authority). MINOR rides the secondary-vow lane (#2641,
  ADR-0159) rather than a parallel guest tier — see ADR-0213. DURANCE-only
  (`MinorStandingDuranceOnlyError`); may only engage the secondary lane
  (`MinorStandingRequiresSecondaryEngageError`); the one exception in
  `validate_secondary_engage_rules` — a MINOR row's secondary vow may stand
  alone with no engaged primary ("a guest's minor vow may be their only lit
  vow"); bypasses `add_member`'s level-band gate; transitions via
  `swear_core`/`step_back_to_minor`.
- **Two-pass auto-engage** — `_auto_engage_durance` now fills the PRIMARY slot
  from CORE-standing memberships and, independently, a vacant SECONDARY slot
  from MINOR-standing memberships; manual engagement still sticks per lane.
- **Attrition auto-dissolve REMOVED** — see "Covenant Exit Lifecycle (#519)"
  above (ADR-0042 amendment). `leave_covenant`/`kick_member` never dissolve
  the covenant; only formation still enforces the two-founder floor.
- **Induction + API** — the induction session's role picker gained a
  `standing` select (defaults CORE); `CharacterCovenantRoleSerializer` exposes
  `standing`; the frontend shows a Minor Member badge.
- **Covenant Treasury** — `world.covenants.treasury` wires the existing
  `OrganizationTreasury` (`world.currency`) that every Covenant's backing
  `Organization` already carries: `covenant_treasury(covenant)`,
  `deposit_covenant_funds` (any active member, core or minor),
  `withdraw_covenant_funds` (rank-gated: `membership.rank.tier <=
  treasury.spend_rank_max`, default 1 = Founder tier). Two new REGISTRY
  Actions (`deposit_covenant_funds`/`withdraw_covenant_funds`), telnet
  `covenant deposit/withdraw`, a member-gated `treasury_balance` field on the
  covenant API payload, and a frontend `TreasuryPanel`.
- See `docs/systems/covenants.md`'s "Minor covenant membership" and "Covenant
  Treasury" sections for the full mechanical writeup, and ADR-0213.
