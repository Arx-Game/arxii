...
def _apply_entrance_flourish_offer(
    actor: ObjectDB, template: ActionTemplate, result: ActionResult
) -> None:
    ...
    if not (template.grants_entry_flourish and result.success):
        return
...
def _apply_entrance_showcase_stake(actor: ObjectDB, result: ActionResult) -> None:
...
class EntranceAction(_SocialTemplateAction):
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        _apply_entrance_flourish_offer(actor, template, result)
        _apply_entrance_showcase_stake(actor, result)
