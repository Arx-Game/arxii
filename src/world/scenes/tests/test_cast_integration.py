...
# ---------------------------------------------------------------------------
# Shared factory helpers — readable seed data doubles
# ---------------------------------------------------------------------------


def _benign_castable_technique() -> Technique:
    """A benign, standalone-castable technique.
...
def _grant(persona: Persona, technique: Technique) -> None:
    """Grant *technique* to *persona*'s CharacterSheet so knows-check passes."""
    CharacterTechniqueFactory(character=persona.character_sheet, technique=technique)


# ---------------------------------------------------------------------------
# Base class — shared room + scene + personas + anima + vitals setup
# ---------------------------------------------------------------------------


class _BaseCastIntegrationTest(TestCase):
    """Shared fixture: check system, room, scene, two personas with anima + vitals."""

    @classmethod
    def setUpTestData(cls) -> None:
        CheckSystemSetupFactory.create()
        room = create_object("typeclasses.rooms.Room", key="Cast Test Room", nohome=True)
