...
def ensure_menace_target() -> None:
    """Look up the ``menace`` ModifierTarget — allure's fear-facing sibling (#2632).
    ...
    check-contribution seam — no new code.
    ...
    on. No-ops when either isn't authored.
    """
    from world.checks.models import CheckType  # noqa: PLC0415
    ...
    target = authored_or_sample(
        ModifierTarget,
        {"category": category, "target_check_type": intimidation},
        name="menace",
    )
    if target is None:
        return
    if target.target_check_type_id is None and intimidation is not None:
