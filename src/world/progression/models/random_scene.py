...
Players receive 5 weekly target characters to RP with. Claiming a target
...
class RandomSceneTarget(SharedMemoryModel):
    """A weekly target character assigned to a player for the Random Scene system.
    ...
    def __str__(self) -> str:
        return (
            f"RS target: {self.account} → {self.target_character}"
            f" (week {self.week_start}, slot {self.slot_number})"
        )


class RandomSceneCompletion(SharedMemoryModel):
    """Permanent record of an account completing a Random Scene with a target character.

    Only one completion record exists per (account, target_character) pair,
    ...
    def __str__(self) -> str:
        return f"RS completion: {self.account} → {self.target_character}"
