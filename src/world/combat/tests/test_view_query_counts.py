...
class EncounterRetrieveQueryCountTests(_SharedSetupMixin, TestCase):
    ...
    def test_warm_retrieve_query_count(self) -> None:
        ...
        #   8. PendingOpponentAttack lookup for ``pending_attacks`` (#3572) - one
        ...
        #   9. PendingSelection lookup for specialist choices; 10. sustained
        #      action lookup for deferred actions; 11. cover/interpose action
        #      lookup for ally-protection state. These three bounded queries
        #      back the bridge acceptance surfaces and do not scale with the
        #      participant count.
