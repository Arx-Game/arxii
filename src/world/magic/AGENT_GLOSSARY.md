...
**Tradition**:
A school of magical practice. Every character has exactly one, the self-taught `Unbound` tradition is a real Tradition row, not a NULL special-case. A Tradition gates which Gifts are pickable at CG via `TraditionGiftGrant` rows (tradition × gift, plus a `special_techniques` M2M of that tradition's unique extras for the gift); there is no selection gate, every tradition a Beginning's slate offers is selectable outright, but `BeginningTradition.state` (`TraditionState`: SELF_TAUGHT/TEACHERS_GONE/LIVING_MASTERS, #3675, was `required_distinction` pre-#3675) decides which drawback the pick carries for free via its `TraditionStateLine`. Organizations may serve as a Tradition's teaching structure, many orgs per tradition (chapters, academies), via the nullable `Organization.tradition` FK (specific to general, ADR-0010; `Tradition` itself stays dependency-free). (#2426, ADR-0136; #3675.)
...
**Tradition Training**:
A rankable distinction that scales a character's CG starting-technique-pick budget: baseline (Unbound, no distinction) is 1 pick, and each rank of Tradition Training adds +1 (rank 1 → 2 picks, rank 2 → 3). Data-modeled as a `ModifierTarget` ("Starting Technique Picks") + `DistinctionEffect`, read at draft time via the existing `CharacterDraft._get_distinction_bonus`, no new mechanism. A LIVING_MASTERS tradition offers it as a `DistinctionOffer` (chapter `tradition_step`) tied to its `SchoolingLine` stances (#3675), not a `BeginningTradition.required_distinction` FK. (#2426, ADR-0136; #3675.)

**Unbound (drawback)**:
The `Distinction` (slug `unbound`, negative `cost_per_rank`) carried for free by picking the Unbound Tradition. The SELF_TAUGHT `TraditionStateLine.carries` FK names it (#3675); `reconcile_offer_picks` applies it to the draft the moment a SELF_TAUGHT tradition is selected, through the same generic mechanism every carried drawback uses (no Unbound-specific code path; Orphaned Tradition's TEACHERS_GONE drawback carries the identical way). Its `DistinctionEffect` carries a +50% surcharge on the `magic_learning_ap_cost` `ModifierTarget`: self-taught mages pay more Action Points to learn techniques (`charge_and_learn`'s AP scale, both front doors), TIME, not power; resonance is untouched. Shed automatically on joining a living Tradition (`join_tradition`), re-applied on leaving one (`leave_tradition` via `world.character_creation.offers.self_taught_drawback()`), the underlying `CharacterModifier` cascade-deletes with the distinction, so the surcharge tracks membership with no separate cleanup. (#2441, #2442, #3675.)
...
**Effect Summary**:
What a Technique actually does, derived on read from its four payload tables (capability grants, applied conditions, damage profiles, removed conditions) plus the existing `is_technique_hostile` / `derive_target_relationship` derivations — never a stored field, since `Technique.target_type`'s help text already rules a stored relationship out. Built by `summarize_technique_effects`, cached on the row as `Technique.cached_effect_summary`, serialized once by `TechniqueEffectSummarySerializer`, and embedded as `effect_summary` by every technique surface (CG, the magic API, the in-scene cast list, the character sheet) so the four cannot drift apart again. Carries a **plain-words line** (`summary`) authored server-side — the same sentence in the web client and over telnet — alongside the structured lists. (#2898.)
_Avoid_: re-deriving or re-wording the sentence client-side; adding an `is_self_buff`-style stored relationship field.

**Underspecified Technique**:
A Technique with no applied condition, no removal, and no damage profile, so nothing about its effect — including its targeting relationship — is derivable from authored data (86 of 272 when #2898 shipped). Surfaced to players as "effects not yet catalogued" rather than a blank, and to staff via `TechniqueAdmin`'s authoring-gap filter. Distinct from an **ambiguous** technique, whose payload rows carry more than one `target_kind` so the single derived relationship is a guess — the self-teleport-that-Flanks-an-enemy shape found in #2764. Both are reported, never guessed around. (#2898.)
