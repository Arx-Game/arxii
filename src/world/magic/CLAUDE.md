...
- `TechniqueStyle` - How a **practitioner** works magic (Manifestation, Subtle, Performance, Prayer, Incantation). A property of the caster's `Path` (`classes.Path.style`), NOT of the technique (#2700, ADR-0167). `StyleCapabilityRequirement` rows let a style gate casting on the caster's capabilities (an Incantation caster who cannot speak cannot incant), evaluated by `technique_performable`. A Gift may set its own style (`Gift.style`, nullable), which overrides this for that gift's techniques — a species gift the character never chose doesn't inherit the Path's manner (#2905, ADR-0197).
