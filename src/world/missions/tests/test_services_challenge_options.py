"""Tests for challenge_options_for_character (Phase A, Task A4).

A CHALLENGE-sourced MissionOption references one ChallengeTemplate; the
challenge's ChallengeApproaches contribute options: one per approach the
acting character qualifies for (they hold the approach's
Application.capability) plus every is_default approach (offered to everyone,
capability-independent). Capability ownership is granted the standard way —
an active condition carrying a ConditionCapabilityEffect — mirroring
test_resolvers.ConditionCapabilityResolverTests.
"""
...
class ChallengeOptionsForCharacterTests(TestCase):
    ...
    def test_character_with_capabilities_gets_every_approach(self) -> None:
        options = challenge_options_for_character(self.challenge, self.char_with)
        self.assertEqual(
    ...
    def test_auto_succeeds_is_carried_and_gated_by_capability(self) -> None:
        ...
        with_options = {
            o.approach.pk: o
            for o in challenge_options_for_character(self.challenge, self.char_with)
        }
        ...
        without = challenge_options_for_character(self.challenge, self.char_without)
    ...
    def test_option_carries_check_type_and_owner(self) -> None:
        [option] = challenge_options_for_character(self.challenge, self.char_without)
    ...
    def test_empty_when_no_default_and_no_capability_match(self) -> None:
        ...
        self.assertEqual(challenge_options_for_character(bare_challenge, self.char_without), [])
