...
the actual ledger / Beat-completion code will replace. The Rumor stub
*raises* NotImplementedError with a DESIGN reference — the upstream rumor
system does not exist yet, so authoring a mission that emits those lines
must fail loudly during apply. (Crime-watch went live with #1765 — see
``world.missions.integrations.crime_watch`` and its report-journey tests.)
"""
...
from world.missions.integrations import beat_stub, money_stub, rumor_stub
