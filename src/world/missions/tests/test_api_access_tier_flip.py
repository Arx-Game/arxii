...
class AccessTierFlipTests(TestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        cls.publishable_giver = MissionGiverFactory(
            name="ready-giver", giver_kind=GiverKind.ROOM_TRIGGER, target=room
        )
        cls.drafty_giver = MissionGiverFactory(
            name="drafty-giver", giver_kind=GiverKind.NPC, target=None
        )
    ...
    def _url(self, template_pk: int) -> str:
        return f"/api/missions/templates/{template_pk}/"

    def test_flip_to_open_succeeds_when_all_givers_publishable(self) -> None:
        template = MissionTemplateFactory(name="ready-tmpl", access_tier=AccessTier.STAFF_ONLY)
        self.publishable_giver.templates.add(template)
        response = self.client.patch(
            self._url(template.pk),
    ...
    def test_flip_to_open_refused_when_any_giver_drafty(self) -> None:
        template = MissionTemplateFactory(name="drafty-tmpl", access_tier=AccessTier.STAFF_ONLY)
        ...
        # The error message names the drafty giver's name so the Studio
    ...
    def test_flip_to_open_with_no_givers_passes(self) -> None:
        ...
        template = MissionTemplateFactory(name="no-givers-tmpl", access_tier=AccessTier.STAFF_ONLY)
        response = self.client.patch(
            self._url(template.pk),
            {"access_tier": AccessTier.OPEN},
            format="json",
    ...
    def test_flip_to_staff_only_always_allowed(self) -> None:
        ...
        template = MissionTemplateFactory(name="unpublish-tmpl", access_tier=AccessTier.OPEN)
        self.drafty_giver.templates.add(template)
        response = self.client.patch(
            self._url(template.pk),
