...
def _on_witness_reaction(window: ReactionWindow, reaction: WindowReaction) -> None:
    ...
    if room is None:
        # Unlike flag_crime (whose caller skips the CRIME_WATCH line entirely
        # when it has no room context), a WITNESS window always has a persona
        # to fall back to: Scene.location is nullable, so when the scene
        # itself carries no location, report at the accused's own current
        # whereabouts instead of silently dropping the report.
...
def open_witness_window(*, interaction: Interaction, entry: LegendEntry) -> ReactionWindow:
    """Idempotently open a WITNESS window on ``interaction`` for bystanders reacting to
    ``entry``.
    """
    from world.justice.models import WitnessReactionTarget  # noqa: PLC0415
    ...
    WitnessReactionTarget.objects.get_or_create(window=window, defaults={"legend_entry": entry})
    return window
