...
class ConsequenceEffect(NaturalKeyMixin, CreditedContent, SharedMemoryModel):
    ...
    **Lore-authorable (#3071):** natural key is ``(consequence, execution_order)``
    ...
    # FK direction per ADR-0010: this consumer points at the justice CrimeKind
    # primitive; justice never imports checks.
    crime_kinds = models.ManyToManyField(
        "arxii.CrimeKind",
        blank=True,
        related_name="consequence_effects",
        help_text=(
            "Crime kinds the awarded deed is an instance of (LEGEND_AWARD only)."
            " A tagged deed accrues pursuit heat as word spreads and can open a"
            " bystander WITNESS reaction window."
        ),
    )
