...
- **`Gender`** / **`Pronouns`**: Canonical lookup tables - SharedMemoryModel. `Gender.is_cg_selectable` (#4022) is what character generation lists and accepts; a row staff keep for NPCs or a future disguise "Indeterminable" is authored with it off. A sheet with no gender is the unknown case.
