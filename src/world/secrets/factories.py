...
from world.secrets.models import (
    Leverage,
    Secret,
    SecretCategory,
    SecretKnowledge,
...
class LeverageFactory(factory_django.DjangoModelFactory):
    class Meta:
        model = Leverage
        django_get_or_create = ("holder_sheet", "subject_sheet", "founded_on")

    holder_sheet = factory.SubFactory(CharacterSheetFactory)
    subject_sheet = factory.SubFactory(CharacterSheetFactory)
    founded_on = factory.SubFactory(SecretFactory)
