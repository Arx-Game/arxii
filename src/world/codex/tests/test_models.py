...
class CodexTeachingOfferCancelTests(CodexTeachingOfferTestCase):
    ...
    def test_cancel_restores_banked_ap(self):
        ...
        pool = ActionPointPoolFactory(
            character=self.teacher.sheet_data,
            current=100,
            maximum=200,
            banked=50,
        )
    ...
    def test_cancel_deletes_offer(self):
        ...
        ActionPointPoolFactory(character=self.teacher.sheet_data, banked=50)
        offer = CodexTeachingOfferFactory(
            teacher=self.teacher,
            entry=self.entry,
            banked_ap=50,
...
class CodexTeachingOfferCanAcceptTests(CodexTeachingOfferTestCase):
    ...
    def test_cannot_accept_own_offer(self):
        ...
        ActionPointPoolFactory(character=self.teacher.sheet_data, current=100)
        offer = CodexTeachingOfferFactory(
            teacher=self.teacher,
            entry=self.entry,
        )
    ...
    def test_cannot_accept_if_already_known(self):
        ...
        ActionPointPoolFactory(character=self.learner.sheet_data, current=100)
        CharacterCodexKnowledgeFactory(
    ...
    def test_cannot_accept_without_ap(self):
        ...
        ActionPointPoolFactory(character=self.learner.sheet_data, current=5)
        offer = CodexTeachingOfferFactory(
            teacher=self.teacher,
            entry=self.entry,
        )
...
class CodexTeachingOfferAcceptTests(CodexTeachingOfferTestCase):
    ...
    def test_accept_spends_learner_ap(self):
        ...
        ActionPointPoolFactory(character=self.teacher.sheet_data, banked=50)
        pool = ActionPointPoolFactory(character=self.learner.sheet_data, current=100)
        offer = CodexTeachingOfferFactory(
            teacher=self.teacher,
            entry=self.entry,
            banked_ap=50,
    ...
    def test_accept_raises_on_invalid(self):
        ...
        ActionPointPoolFactory(character=self.learner.sheet_data, current=5)  # Insufficient
        offer = CodexTeachingOfferFactory(
            teacher=self.teacher,
            entry=self.entry,
        )
