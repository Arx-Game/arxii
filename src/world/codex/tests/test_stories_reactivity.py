...
service when status transitions to KNOWN (#939 — the hook lives on the model
method so no caller can bypass it), so active stories with
...
class CodexAddProgressReactivityTests(EvenniaTestCase):
    def test_known_transition_flips_codex_beat(self) -> None:
        ...
        # Flip to KNOWN — the hook fires on the model method itself.
