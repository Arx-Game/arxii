...
- **`weather.py`**: `CmdTime` (`time`, alias `weather`, #1522) — read-only IC time + local-weather
  glance. Thin over `world.weather.services.current_conditions(room)`: shows the IC clock
  (time/phase/season) and the room's effective weather + one season/phase-appropriate emit line
  (the same data the periodic ATMOSPHERE echo pushes and the React widget renders). No action.
