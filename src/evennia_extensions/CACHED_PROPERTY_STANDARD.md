...
- **`to_attr=` onto anything else remains rejected** — a plain `@property` or bare attribute never satisfies Django's `isinstance` check at all (a getter that never raises defeats the `hasattr` fallback, so the batched query silently never runs), and a `cached_property`/`CachedRowsHandler`-style wrapper with no invalidation wired batches once and then never refreshes (ADR-0263's original finding). `CachedRowsHandler` is retired as its consumers migrate to `PrunedCachedProperty`; a new `to_attr` outside that migration still fails the repo-wide `pattern:PREFETCH_TO_ATTR` ratchet, and the `prefetch-to-attr` hook covers the surfaces already converted.
