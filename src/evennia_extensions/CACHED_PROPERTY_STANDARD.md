...
- **`to_attr=` onto anything else remains rejected, for three distinct failure modes:**
  ...
  `CachedRowsHandler` is retired; all current consumers use `PrunedCachedProperty`, and a new `to_attr` outside that pattern still fails the repo-wide `pattern:PREFETCH_TO_ATTR` ratchet. **The `prefetch-to-attr` hook does not cover the converted surfaces yet** — its `files:` scope (`evennia_extensions/`, `web/admin/upbringing_builder/`) contains zero `to_attr` call sites today, only prose mentions, so a clean run of that hook is not evidence the converted properties elsewhere are enforced. Widening it to the apps this plan actually converted (`world/scenes` and others) is blocked on #3835: most of those `to_attr` sites split the `Prefetch` call from the `PrunedCachedProperty` definition across files (the dominant, correct layout), and the hook's same-file heuristic cannot see across that split, so widening naively would false-positive on already-correct code before it ever reached the genuinely unconverted sites.

The cached_property rule above still stands for ordinary non-prefetch caches. A request-specific or parameterized view should first use a prefetched raw `PrunedCachedProperty` list plus a cheap derived property over that list. Do not introduce a handler merely to filter a parent-owned list; `CachedRowsHandler` was removed once its final consumer was converted (#3835).
