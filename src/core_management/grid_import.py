...
6. **Clue/anchor sidecars** — ``RoomClue``/``ClueTrigger``/``PortalAnchor``
...
``source`` (e.g. ``weather:cold-snap``) is left untouched. ``AmbientEmoteLine``
rows are likewise wholesale-replaced within each bundle's scope (deleted +
recreated), not upserted — see pass 5. The same report-never-delete rule
applies to fixture-keyed ``RoomClue``/``ClueTrigger``/``PortalAnchor`` rows
