...
only copy of our authored content (ADR-0237/ADR-0238) and is never dropped, full stop.
...
- **Data migrations are REQUIRED where authored content is at risk** (ADR-0237, superseding ADR-0013): as of alpha the production database holds authored content — codex entries, lore prose, techniques, traditions, conditions, check types, catalogs, grid rooms — that exists nowhere else. Any `RemoveField`/`DeleteModel` on an authored-content table **must** declare its data disposition in the PR: *restructure* (a `RunPython` in the same migration carries the data across — mandatory), *deliberate discard* (stated, signed off, recoverable only from backup), or *empty in production* (a claim checked against prod, never assumed from a dev database). `RenameField` is not data loss — the data survives — though it stays flagged on the separate rollback-compatibility axis (`docs/operations/observability-baseline.md` §4.5). Deploy applies `migrate --noinput` unattended on every converge, so the merged migration is itself the destructive act; PR review is the only gate. Play-state tables (characters, sheets, XP, scenes, encounters) are alpha-resettable and still need no backfill.
...
- **`# noqa` Suppression Policy**: `# noqa` comments for our custom linters should be rare exceptions, not a convenient escape hatch. Only suppress when fixing the violation would cause more harm than good — for example, necessitating a massive and inelegant refactor. Every suppression MUST include a brief justification comment explaining why (e.g., `# noqa: SHARED_MEMORY — abstract mixin used by multiple apps`). Custom linter tokens: `PREFETCH_STRING`, `STRING_LITERAL`, `SHARED_MEMORY`, `USE_FILTERSET`, `GETATTR_LITERAL`, `CACHED_PROPERTY_IMPORT`, `OBJECTDB_PARAM`, `IDMAPPER_MUTATE_ORDER`, `IDMAPPER_ONLY`, `VIEW_MEMO`. `VIEW_MEMO` (`view-memo` hook): per-request state kept on a view/serializer — either a None-defaulted private class attribute or the lazy `hasattr(self, "_x")` / `self._x = ...` form inside a method; only constant configuration earns a suppression, a memo never does (ADR-0260, #3597, #3622)

  **Suppression counts are ratcheted** (`tools/lint_noqa_ratchet.py` + `tools/noqa_ratchet_baseline.txt`, enforced by the `noqa-ratchet` pre-commit hook): the per-token count of `# noqa` suppressions under `src/` may only go down. Adding a new suppression fails the hook until the baseline is deliberately bumped in the same commit with a justification — build the missing scaffolding instead of silencing the linter. When an audit tranche retires suppressions, lower the baseline in the same PR to lock the progress in. (Currently ratcheted: `GETATTR_LITERAL`; `IDMAPPER_ONLY` is pinned at zero.)
...
- **Never bulk-`.update()`/`.bulk_update()` a `SharedMemoryModel`**: Evennia's idmapper intercepts model construction (`SharedMemoryModelBase.__call__`) and returns the cached in-memory instance for a pk instead of the freshly-queried row whenever one is already resident — but bulk `.update()`/`.bulk_update()` write directly to the DB via raw SQL and never go through `.save()`, so they never invalidate that cache. A cached instance elsewhere in the process (or in the same test, via a factory-held reference) keeps reading the stale value even after the bulk write lands in Postgres. When you need to flip a field on N rows, loop and call `.save(update_fields=[...])` on each fetched/cached instance instead — it costs N writes instead of 1, but it's the only path that actually updates what the rest of the process sees. (Found the hard way in #1899: a bulk `MissionInstance.objects.filter(pk__in=ids).update(is_paused=True)` silently left already-cached instances reporting `is_paused=False`.)
- **Never `.only()`/`.defer()` a `SharedMemoryModel`** (ADR-0261): the same interception is why a narrowed load is unsafe. A row whose *first* load in a process goes through `.only(...)` is resident with the other columns missing, and every later full-column query hands back that same narrowed instance without copying the columns onto it. The next read of a missing field goes through Django's deferred-attribute getter, whose `refresh_from_db(fields=[...])` also gets the resident instance back, skips the still-deferred field and raises `KeyError`. That was Sentry ARX2-9 (2026-09-03): the CG beginnings list prefetched codex grants with `.only("beginnings_id", "entry_id")` and the Beginnings admin change page 500'd on `is_perspective` until restart. Nothing is saved by narrowing a row the identity map loads once. Drop the `.only`/`.defer`; if a projection is truly wanted (pk walks, counts), use `.values()`/`.values_list()`, which never instantiate a model. Enforced by `tools/lint_only_defer.py` (`lint-only-defer` hook, `# noqa: IDMAPPER_ONLY` with a reason, ratcheted at zero).
...
## Generated API Schema (drf-spectacular)

Two committed files are generated from the DRF backend by `just gen-api-types`: `src/schema.json` and `frontend/src/generated/api.d.ts`. Backend Python is the single source of truth; the `.d.ts` is downstream output — never hand-edit it. CI's `api-types-drift` job regenerates the schema from the PR merged with main and fails if either committed file differs.

**What drifts the schema (all require a regen + commit of both files):**
- Any field/type change on a serializer or viewset.
- **Docstring-only edits** — drf-spectacular derives operation/field descriptions from Python docstrings on viewset `@action` methods and `SerializerMethodField` getters, so even a docstring-only edit changes the schema with no type change.
- **Adding a value to a `TextChoices`/`IntegerChoices` enum** that any serializer field exposes — drf-spectacular bakes the enum's choices and per-value help text into the schema's `<Enum>Enum` component. Easy to miss because adding an enum value feels like a constants-only edit.

**The one case where "the merge queue handles it" does NOT apply:** the merge queue (see CLAUDE.md #991) re-tests the PR against latest main at merge time, which covers runtime re-integration and migration ordering — but it does **not** regenerate committed generated files. If main changed any serializer while your PR was open, your committed `schema.json`/`api.d.ts` are now stale relative to main-merged code, and `api-types-drift` fails even though your branch didn't touch serializers. Fix: `git merge origin/main` → `just gen-api-types` (always regenerate — a git auto-merge of `schema.json` is often itself wrong, don't trust it) → commit → push → enqueue promptly (if main moves again before approval it can re-drift).
